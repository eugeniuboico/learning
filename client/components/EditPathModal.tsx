...
export const EditPathModal: React.FC<EditPathModalProps> = ({ isOpen, path, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (path) {
      ...
      setStarsRequired(path.requiredScore?.toString() || '0');
    }
  }, [path]);
  ...
  return (
    <>
      <ConfirmDialog
        ...
        title="Delete Path"
        message={`Are you sure you want to delete "${path?.title}"? This will delete all lessons and tasks in this path.`}
        confirmText="Delete"
        cancelText="Cancel"
        variant="danger"
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex justify-between items-center pt-4">
            <button
              ...
              disabled={isSubmitting}
              className="px-6 py-3 bg-red-600 text-white font-bold rounded-xl hover:bg-red-700 transition-colors shadow-md disabled:opacity-50 flex items-center space-x-2"
            >
